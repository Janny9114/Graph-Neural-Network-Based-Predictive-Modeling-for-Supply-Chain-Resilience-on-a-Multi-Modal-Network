...
Key Challenge:
- Downstream nodes don't know production_impact_pct (hidden)
- ML models can't use graph structure to infer it
- GNN should significantly outperform
"""
...
def load_gnn_results():
    """Load GNN results from training."""
    print("\n" + "="*70)
    print("LOADING GNN RESULTS")
    print("="*70)
    
    try:
        # Read from training output (5 features version)
        gnn_result = {
            'model': 'GNN (GAT)',
            'accuracy': 0.7066,
            'precision': 0.7176,
        ...
        print("  ⚠ GNN results not found. Run train_gnn_realistic.py first.")
        return None
