...
def train_model(model, model_name, train_loader, val_loader, optimizer, device, class_weights, use_edge_attr=True, num_epochs=400, patience=50):
    ...
    # OPTIONAL: Save training history (commented out - not required for website operation)
    # Uncomment the lines below if you want to save training metrics for analysis
...
def main():
    ...
    # ✅ SOFTENED CLASS WEIGHTS: Reduce aggressive weighting
    # Original sklearn weights: [2.936, 4.589, 0.410]
    # Problem: Degraded weight (4.589) too high → over-prediction (precision=45%)
    # Solution: Apply square root to soften weights, then normalize
    class_weights = torch.tensor(class_weight_values, dtype=torch.float)
    
    print(f"  ✓ Original sklearn weights: {class_weights.cpu().numpy()}")
    
    # Soften weights using square root (reduces extreme values)
    class_weights = torch.sqrt(class_weights)
    
    # Normalize to sum to num_classes (maintains relative importance)
    class_weights = class_weights / class_weights.sum() * num_classes
    
    class_weights = class_weights.to(device)
    
    print(f"  ✓ Softened class weights: {class_weights.cpu().numpy()}")
    ...
    for model_name, (model, use_edge_attr) in models.items():
        ...
        # OPTIONAL: Plot training curves (commented out - not required for website operation)
        # Uncomment the line below if you want to generate training curve visualizations
