...
class EdgeDisruptionSimulator(RealisticDisruptionSimulator):
    ...
    def generate_scenarios_with_edge_disruptions(self, G, base_buffers, num_scenarios=10000):
        ...
        for i in tqdm(range(num_scenarios), desc="Simulating scenarios"):
            ...
            # Hybrid scenarios
                ...
                # ✅ FIX: Flatten structure - put initial_node and disrupted_edges at top level
                # This ensures create_pyg_data_objects can find and adjust both node and edge features
                scenarios.append({
                    'scenario_id': i,
                    'scenario_type': 'hybrid_node_edge',
                    'disruption_category': 'hybrid',
                    'initial_node': node_scenario['initial_node'],  # ✅ Top level for node adjustment
                    'initial_impact_pct': node_scenario['initial_impact_pct'],  # ✅ Top level
                    'disrupted_edges': edge_scenario['disrupted_edges'],  # ✅ Top level for edge adjustment
                    'edge_capacity_reduction': edge_scenario['edge_capacity_reduction'],  # ✅ Top level
                    'node_disruption': node_scenario,  # Keep nested for reference/debugging
                    'edge_disruption': edge_scenario,  # Keep nested for reference/debugging
    ...
    def create_pyg_data_objects(self, G, node_df, edge_df, scenarios):
        ...
        for scenario in tqdm(scenarios, desc="Creating Data objects"):
            ...
            # Fill in scenario-specific node data for ALL nodes (including unaffected)
            for node_id in range(num_nodes):
                if node_id in scenario['results']:
                    # Affected node: use computed label
                    y[node_id] = scenario['results'][node_id]['label']
                else:
                    # Unaffected node: label as Normal
                    y[node_id] = 2
                
                # Train on ALL nodes (no masking)
