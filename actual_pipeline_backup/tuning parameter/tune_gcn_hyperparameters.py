"""
GCN Hyperparameter Tuning for Supply Chain Disruption Scenarios

This script performs comprehensive hyperparameter tuning for GCN (Graph Convolutional Network)
...
3. dropout: Dropout rate for regularization
4. learning_rate: Optimizer learning rate
5. weight_decay: L2 regularization strength
...
def tune_hyperparameters(train_data, val_data, n_trials=50):
    ...
    # Collect all unique labels across ALL scenarios
    all_labels = []
    ...
    # Safety check: Force to 3 if only 1 class detected
    if num_classes == 1:
    ...
    with open('gcn_best_hyperparameters.json', 'w') as f:
        json.dump(best_params, f, indent=2)
    
    print(f"\n✓ Saved best hyperparameters to: gcn_best_hyperparameters.json")
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig('gcn_tuning_results.png', dpi=300, bbox_inches='tight')
        print(f"✓ Saved tuning plots to: gcn_tuning_results.png")
        plt.close()
...
def test_best_model(best_params, train_data, val_data, test_data):
    ...
    for epoch in range(200):  # Max 200 epochs for final model
        ...
        if val_acc > best_val_acc:
            ...
            patience_counter = 0
            # Save best model
            torch.save(model.state_dict(), 'best_gcn_tuned.pt')
        else:
            patience_counter += 1
            if patience_counter >= patience:
    ...
    model.load_state_dict(torch.load('best_gcn_tuned.pt', weights_only=True))
    ...
    with open('gcn_tuning_final_results.json', 'w') as f:
        json.dump(final_results, f, indent=2)
    
    print(f"\n✓ Saved final results to: gcn_tuning_final_results.json")
    print(f"✓ Saved best model to: best_gcn_tuned.pt")
...
def main():
    ...
    print("="*70)
    print("GCN HYPERPARAMETER TUNING FOR SUPPLY CHAIN DISRUPTIONS")
    print("="*70)
    ...
    print("\n📁 Output Files:")
    print("  - gcn_best_hyperparameters.json")
    print("  - gcn_tuning_final_results.json")
    print("  - best_gcn_tuned.pt")
    print("  - gcn_tuning_results.png")
