"""
GNN Training for Supply Chain Resilience Prediction
Trains a Graph Attention Network (GAT) to predict node resilience using disruption simulation labels
"""
...
def load_data(labels_file='node_resilience_labels_cascading.csv', graph_file='supply_chain_graph_cascading_drnl.pt'):
    """Load graph data and resilience labels."""
    print("Loading data...")
    
    # Load graph (with DRNL features)
    ...
    # Load resilience labels (CASCADING by default)
    labels_df = pd.read_csv(labels_file)
    print(f"  Labels loaded from: {labels_file}")
    print(f"  Total nodes: {len(labels_df)}")
...
def plot_training_history(train_losses, val_accuracies, val_f1_scores):
    ...
    axes[2].set_title('Validation F1 Score Over Time')
    axes[2].legend()
...
def main():
    ...
    # Load data with CASCADING labels and DRNL features
    data = load_data('node_resilience_labels_cascading.csv', 'supply_chain_graph_cascading_drnl.pt')
    ...
    ml_results = train_baseline_ml(X_train, y_train, X_test, y_test)
